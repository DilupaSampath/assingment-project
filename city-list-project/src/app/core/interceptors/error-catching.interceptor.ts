        ...
        return next.handle(request)
            .pipe(
                ...
                catchError((error: HttpErrorResponse) => {
                    let errorMsg: any = [];
                    if((error.status === 504) || (error?.statusText?.indexOf('Gateway Timeout') > -1) || (error?.statusText?.indexOf('Service Unavailable') > -1)){
                    ...
                    } else if(error?.error?.message?.join("")?.indexOf("Bad credentials") > -1){
                        this.globalFacadeService.notificationService.openSnackBar('Invalid username or password', 'red-snackbar');
                    } else {
