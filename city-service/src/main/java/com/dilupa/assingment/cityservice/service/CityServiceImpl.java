...
public class CityServiceImpl implements ICityService {
     ...
     * @throws Exception
     */
    public Page<City> getAllWithPagination(int offset, int pageSize, String filter) throws Exception {
        Page<City> cityPage;
        if(Optional.ofNullable(filter).isPresent()){
            cityPage = cityRepository.findByNameContaining(filter, PageRequest.of(offset, pageSize));
        }else{
            cityPage = cityRepository.findAll(PageRequest.of(offset, pageSize));
        }
        return cityPage;
    }
}
